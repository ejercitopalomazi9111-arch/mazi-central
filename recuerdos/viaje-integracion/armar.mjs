...
//   salida/{mujer,hombre}-{frente,reverso}.png   una tarjeta, 44 × 63.6 mm a 300 ppp
//   salida/imprimir.pdf   carta horizontal, 15 por cara (5 × 3): compañeras (frente,
//                         reverso) y luego compañeros (frente, reverso)
// Si alguna versión pasa de 15:  node armar.mjs --mujer 2 --hombre 1  (hojas de cada una)
...
const url = (ver, extra = '') => pathToFileURL(path.join(aqui, 'tarjeta.html')).href + '?ver=' + ver + extra;
const arg = (n) => { const i = process.argv.indexOf('--' + n); return i > 0 ? process.argv[i + 1] : '1'; };
...
for(const v of ['mujer', 'hombre']) for(const ver of ['frente', 'reverso']){
  await pagina.goto(url(ver, '&v=' + v));
  ...
  const fuera = await pagina.evaluate(() => {
    ...
    return [...document.querySelectorAll('.cara *:not(.mancha):not(.adorno):not(.adorno *):not(.punto):not(.cinta-adhesiva):not(.marca-agua):not(.camion):not(.camion *)')]
  ...
  if(fuera.length) errores.push(`${v} ${ver}: se sale ${fuera.join(', ')}`);
  ...
  const cortado = await pagina.evaluate(() => [...document.querySelectorAll('.cara *')]
    ...
    .map((el) => el.getAttribute('class') || el.tagName));
  if(cortado.length) errores.push(`${v} ${ver}: queda cortado ${cortado.join(', ')}`);
  await pagina.locator('.cara').screenshot({ path: path.join(salida, `${v}-${ver}.png`) });
  console.log('✓', `${v}-${ver}.png`);
}

const hojas = await navegador.newPage();
await hojas.goto(url('hojas', `&mujer=${arg('mujer')}&hombre=${arg('hombre')}`));
