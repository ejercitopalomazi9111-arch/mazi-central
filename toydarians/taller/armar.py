...
/* ── El conmutador de idioma ─────────────────────────────────────────────
   Dos letras, no una banderita: una bandera dice «país» y el idioma no es un
   país —el inglés de un coleccionista de Manchester y el de uno de Texas es
   el mismo—. Y con `aria-pressed` se anuncia como lo que es: un interruptor
   que está apagado o encendido, no un enlace a otra página. */
.g-idioma{margin-left:auto;border:1px solid var(--linea);background:transparent;
  color:var(--gris-tenue);font:700 10px/1 var(--dato);letter-spacing:.14em;
  padding:9px 11px;cursor:pointer;transition:color .18s ease,border-color .18s ease}
.g-idioma:hover,.g-idioma:focus-visible{color:var(--amarillo);border-color:var(--amarillo)}
.g-idioma[aria-pressed="true"]{color:var(--tinta);background:var(--amarillo);
  border-color:var(--amarillo)}
@media (max-width:560px){.g-idioma{padding:8px 9px}}
...
/* ⚠ EN TELEFONO ESTE BOTON SE QUEDABA VACIO. Debajo de 420 px se esconde la
   palabra «Carrito», y con el carrito vacío tambien se esconde el contador:
   quedaba un recuadro amarillo con NADA dentro, que no parece un boton sino un
   fallo de carga. Se veia solo en telefono y con el carrito vacio —o sea, en
   la primera visita de todo el mundo—. Ahora, cuando la palabra no cabe, entra
   una marca de carrito en su lugar, y nunca queda hueco. */
@media (max-width:420px){
  .g-carro-btn .txt{display:none}
  /* y con la palabra fuera, el contador SE QUEDA aunque marque cero: es lo
     único que le da contenido al botón, y un «0» es información —el carrito
     está vacío— mientras que un recuadro en blanco es un error. */
  .g-carro-btn[data-vacio="si"] .n{display:inline-grid;opacity:.55}
  .g-carro-btn{padding:10px 12px}
}
...
# ══ EL CONMUTADOR DE IDIOMA ═══════════════════════════════════════════════
# Lo pidio Luis: «pon un boton que traduzca todo a ingles».
#
# Por que un diccionario y no llamar a un traductor: este sitio se sube a una
# carpeta y tiene que funcionar sin servidor —asi se lo entregamos al cliente—.
# Un traductor en linea necesita red, llave y dinero por peticion, y traduce
# distinto cada vez: «El cartón» acabaria siendo «the cardboard» en vez de «the
# card», que es la palabra del coleccionista. Con diccionario, la traduccion la
# escribe quien sabe de esto UNA vez y no cambia nunca.
#
# Lo que NO se traduce, y es a proposito: los nombres de las figuras, las
# series y las marcas (Dengar, The Bad Batch, Vintage Collection) ya estan en
# ingles y son nombres propios. Traducirlos seria estropearlos.
ES_EN = {
    # barra y menu
    'La vitrina': 'The showcase', 'Categorías': 'Categories',
    'El cartón': 'The card', 'Contacto': 'Contact', 'Carrito': 'Cart',
    'Menú': 'Menu', 'Marcas y líneas': 'Brands & lines', 'La tienda': 'The shop',
    'Ver todas las piezas': 'See every piece', 'Tu carrito': 'Your cart',
    '¿Buscas algo? Escríbenos': 'Looking for something? Write us',
    'Inicio · Vintage Collection': 'Home · Vintage Collection',
    'Figuras': 'Figures', 'Packs': 'Packs', 'Ver la categoría': 'See the category',
    'Próximamente': 'Coming soon', 'Saltar': 'Skip',
    # portadas
    'The Vintage Collection · Escala 3.75&Prime;':
        'The Vintage Collection · 3.75&Prime; scale',
    'Para verdaderos': 'For serious', 'coleccionistas': 'collectors',
    'Funko Pop! · Vinilo de 10 cm': 'Funko Pop! · 4 inch vinyl',
    'La cabeza': 'Big head,', 'grande,': 'and', 'en serio': 'dead serious',
    'Impresión 3D · Hecho por encargo': '3D printing · Made to order',
    'El escenario': 'The set piece', 'que': 'they', 'no venden': "don't sell",
    'Ver la vitrina': 'See the showcase', 'Hablemos': "Let's talk",
    'Descubre la Vintage Collection': 'Discover the Vintage Collection',
    'Funko Pop! en Toydarians': 'Funko Pop! at Toydarians',
    'Impresión 3D en Toydarians': '3D printing at Toydarians',
    'una por una': 'one by one', 'Ocho cajas,': 'Eight boxes,',
    'una repisa': 'one shelf', 'Siete piezas': 'Seven pieces',
    'para montar escena': 'to build the scene',
    # vitrina y mando
    'Orden': 'Sort', 'Todas': 'All', 'Nombre A → Z': 'Name A → Z',
    'Nº VC ascendente': 'VC number, low to high',
    'Nº VC descendente': 'VC number, high to low',
    'Como en la tienda': 'As in the shop', 'Al revés': 'Reversed',
    'Precio de menor a mayor': 'Price, low to high',
    'Precio de mayor a menor': 'Price, high to low',
    'Ver a detalle': 'See in detail', 'Precio a consultar': 'Price on request',
    'Ninguna pieza coincide.': 'No piece matches.',
    'Buscar por nombre o número VC…': 'Search by name or VC number…',
    'Buscar por nombre o película…': 'Search by name or film…',
    'Buscar por nombre o serie…': 'Search by name or series…',
    'Buscar': 'Search', 'Borrar la búsqueda': 'Clear the search',
    'Filtrar por serie': 'Filter by series',
    # ficha
    'Expediente': 'File', 'Línea': 'Line', 'Nº de pieza': 'Piece no.',
    'Escala': 'Scale', 'Estado': 'Condition', 'Formato': 'Format',
    'Entrega': 'Delivery',
    'En su cartón original, sin abrir': 'On its original card, unopened',
    'Vinilo · caja con ventana': 'Vinyl · window box',
    'Caja nueva, sin abrir': 'New box, unopened',
    'Impresión 3D por encargo': '3D printed to order',
    'En existencia, o 4–5 días si se imprime':
        'In stock, or 4–5 days if printed',
    'Agregar al carrito': 'Add to cart', 'Disponible': 'Available',
    'Agotado': 'Sold out', 'Agregado ✓': 'Added ✓',
    'Toydarians cotiza esta pieza': 'Toydarians quotes this piece',
    'Pedir precio por WhatsApp': 'Ask the price on WhatsApp',
    'Más de la colección': 'More from the collection', 'Cerrar': 'Close',
    # carrito
    'Total': 'Total', 'Ver mi carrito': 'See my cart',
    'El envío se calcula al pagar, con la dirección que pongas.':
        'Shipping is worked out at checkout, with the address you enter.',
    'Se paga aquí,': 'Pay here,', 'se envía a tu casa': 'delivered to your door',
    'Falta conectar el cobro': 'Payment is not connected yet',
    # contacto
    '¿Buscas algo?': 'Looking for something?',
    'Si no está aquí,': "If it isn't here,", 'pregúntalo': 'just ask',
    'Se contesta en horario de tienda.': 'Answered during shop hours.',
    'Correo': 'Email', 'Teléfono': 'Phone',
    'Lo más rápido. El mensaje ya va escrito.':
        'Fastest. The message is already written.',
    'Para listas largas o fotos.': 'For long lists or photos.',
    'Si prefieres hablarlo.': 'If you would rather talk.',
    # pie
    'Precios en pesos mexicanos. Las existencias son las del catálogo y':
        'Prices in Mexican pesos. Stock is the catalogue’s and',
    'Toydarians confirma cada pedido': 'Toydarians confirms every order',
    'ahora mismo': 'right now', 'verla a detalle': 'see it in detail',
    'sin abrir': 'unopened', 'al revés': 'the other way',
    'en caja': 'boxed', 'precio lo cotiza Toydarians': 'price is quoted by Toydarians',
    'Se compra': 'Bought', 'cerrado': 'sealed',
    'Eliges tus figuras, pagas con tarjeta o con PayPal, y pones tu dirección '
    'en el mismo paso. Sin salir de esta página.':
        'Pick your figures, pay by card or with PayPal, and enter your address '
        'in the same step. Without leaving this page.',
    'Star Wars, The Vintage Collection y las demás marcas son de sus titulares. '
    'Este sitio no está afiliado a ellos.':
        'Star Wars, The Vintage Collection and the other brands belong to their '
        'owners. This site is not affiliated with them.',
    'antes de enviarlo; si algo se agotó, se avisa y se devuelve el importe.':
        'before shipping it; if something sold out, you are told and refunded.',
    'Cada una con sus fotos de la tienda. Toca cualquiera para':
        'Each with its photos from the shop. Tap any of them to',
    'Fotos de la caja y de la figura suelta. Toca cualquiera para':
        'Photos of the box and of the loose figure. Tap any of them to',
    'Fotos de las impresiones reales del taller. Toca cualquiera para':
        'Photos of the workshop’s actual prints. Tap any of them to',
    'Las marcas y líneas del catálogo, con el número de piezas que se pueden '
    'comprar aquí': 'The brands and lines of the catalogue, with how many '
                    'pieces can be bought here',
    'Todo lo que': 'Everything', 'hay en la tienda': 'the shop carries',
    'Quien colecciona no compra la figura: compra el cartón':
        'A collector does not buy the figure: they buy the card',
    'Aquí el brillo se mueve': 'Here the shine moves',
    'Tronos, bóvedas y bases para montar la escena que la figura no trae. '
    'Se imprimen por encargo, así que el':
        'Thrones, vaults and bases to build the scene the figure does not come '
        'with. Printed to order, so the',
    'pieza por pieza.': 'piece by piece.',
    # Los trozos que quedan A LOS LADOS de un <strong>. En el DOM cada uno es
    # su propio nodo de texto, asi que se traducen por separado o no se
    # traducen: un parrafo con negritas no llega nunca entero al diccionario.
    'Aquí el número': 'Here the', 'es la pieza, y el cartón se compra cerrado. '
    'Toydarians tiene': 'number is the piece, and the card is bought sealed. '
                        'Toydarians has',
    'de The Vintage Collection y': 'from The Vintage Collection and',
    'de Hasbro en total.': 'from Hasbro in total.',
    '. La curva del blíster, el troquel del colgadero, el brillo que se corre '
    'al girarlo.': '. The curve of the blister, the die-cut hook, the shine '
                   'that runs as you turn it.',
    'que la tarjeta — es lo que convence de que hay plástico y no un dibujo de '
    'plástico. Arrástrala.': 'than the card — that is what convinces you there '
                             'is plastic and not a drawing of plastic. Drag it.',
    'En esta vitrina hay': 'This showcase holds',
    ', y son una selección. Si buscas una figura que no ves —o quieres el '
    'precio de una impresión 3D— escríbenos y te decimos si la tenemos.':
        ', and they are a selection. If you are after a figure you cannot see '
        '—or want the price of a 3D print— write to us and we will tell you '
        'whether we have it.',
    'No incluye la figura.': 'The figure is not included.',
    'Si no hay existencia se reserva: unos 4 días sin pintar, 5 pintado.':
        'If not in stock it can be reserved: about 4 days unpainted, 5 painted.',
    'No incluye la figura. Si no hay existencia se reserva: unos 4 días sin '
    'pintar, 5 pintado.':
        'The figure is not included. If not in stock it can be reserved: about '
        '4 days unpainted, 5 painted.',
    'Back to the Future casi entera y el Mandalorian con el Niño. Ocho figuras':
        'Nearly all of Back to the Future, plus the Mandalorian with the Child. '
        'Eight figures',
    ', con su ventana y su número de serie, listas para el estante.':
        ', with their window and serial number, ready for the shelf.',
}

# Las frases que llevan un número dentro no caben en un diccionario: «47
# piezas» cambia con el catálogo. Van como reglas, que el conmutador aplica
# cuando ninguna entrada literal coincide.
REGLAS_EN = [
    (r'^(\d+) piezas$', r'\1 pieces'),
    (r'^(\d+) piezas en esta vitrina$', r'\1 pieces in this showcase'),
    (r'^(\d+) piezas · (\d+) en esta vitrina$', r'\1 pieces · \2 in this showcase'),
    (r'^(\d+) piezas,$', r'\1 pieces,'),
    (r'^Ver las (\d+) piezas$', r'See all \1 pieces'),
    (r'^Agregar (.+) al carrito$', r'Add \1 to the cart'),
]
...
COPIA = {
 'vintage-collection': {
   ...
   # Lo pidió Luis: «cambia lo de cada pieza tiene su número a para verdaderos
   # coleccionistas». El titular deja de describir el catálogo y pasa a nombrar
   # a quien compra, que es lo que de verdad vende — el número sigue estando,
   # pero donde le toca: en el párrafo y en la píldora de cada pieza.
   'h1': 'Para verdaderos<br><em>coleccionistas</em>',
   'dicho': f'<p>Aquí el número <strong>VC</strong> es la pieza, y el cartón se '
            f'compra cerrado. Toydarians tiene <strong>{VC_TOTAL}</strong> de '
            f'The Vintage Collection y <strong>{HASBRO_TOTAL}</strong> de '
            f'Hasbro en total.</p>',
...
<div class="barra"><div class="caso">
  ...
  <button class="g-idioma" id="g-idioma" type="button" aria-pressed="false"
          aria-label="Read this site in English"><span>EN</span></button>
...
def escribir_idioma():
    """El conmutador de idioma, en su propio archivo y compartido por las tres.

    Barre los NODOS DE TEXTO y cambia los que coinciden con una entrada del
    diccionario. No lleva marcas en el HTML —ni `data-en` en cada etiqueta— por
    una razon: el marcado tendria que acordarse de traer su traduccion en cada
    sitio donde se escribe, y el dia que alguien añada un boton sin acordarse,
    ese boton se queda en español sin que nada chille. Asi, la unica lista que
    hay que mantener es el diccionario, y la compuerta puede comprobarla entera.

    El original se guarda en el propio nodo la primera vez que se toca, asi que
    volver a español es exacto y no depende de traducir al reves.
    """
    import html as _html
    # Las claves se guardan con el espacio YA COLAPSADO, igual que lo normaliza
    # el conmutador antes de buscar. Sin esto, una clave escrita con espacio
    # duro —«10&nbsp;cm»— no coincidia nunca: en el navegador `\s` tambien casa
    # el espacio duro, asi que la busqueda lo convertia en espacio normal y el
    # diccionario seguia teniendo el duro. El sintoma era una sola frase sin
    # traducir en una sola pagina, que es justo lo que nadie va a mirar.
    def _norm(x):
        return re.sub(r'\s+', ' ', _html.unescape(x)).strip()
    # Las que se escriben igual en los dos idiomas se caen: no traducen nada, y
    # ademas confunden a la compuerta, que busca claves del diccionario sueltas
    # en la pagina en ingles para cazar lo que quedo sin traducir.
    dicc = {_norm(k): _norm(v) for k, v in ES_EN.items()
            if _norm(k) != _norm(v)}
    js = """/* Conmutador de idioma · sin dependencias, sin red, sin servidor. */
(function(){
  var TOY = window.TOY = window.TOY || {}; TOY.g = TOY.g || {};
  var DIC = """ + json.dumps(dicc, ensure_ascii=False, separators=(',', ':')) + """;
  var REGLAS = """ + json.dumps([[a, b.replace('\\1', '$1').replace('\\2', '$2')]
                                 for a, b in REGLAS_EN],
                                ensure_ascii=False, separators=(',', ':')) + """;
...
"""
    (RAIZ / 'idioma.js').write_text(js, encoding='utf-8')
    return len(dicc)
...
def escribir(pag):
    ...
    # El idioma va DESPUÉS del motor: traduce lo que el motor ya pintó, y se
    # queda observando para traducir la ficha y el carrito, que se arman luego.
    doc += '<script src="idioma.js" defer></script>\n'
...
print(f'idioma.js      {escribir_idioma():>3} frases traducidas')
for pag in PAGINAS:
