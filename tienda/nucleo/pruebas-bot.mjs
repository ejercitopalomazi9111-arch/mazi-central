...
const tope = charla(['quiero 5 cera brillo']);
ok('pide 5 y quedan 3: pone 3 y lo dice', tope[0].estado.carrito[0]?.[1] === 3 && /Sólo me quedaban 3/.test(tope[0].texto), tope[0].texto);
const mucho = charla(['quiero 999999 navajas', 'si']);
ok('pide 999999: no le pone todo el anaquel, pregunta', !mucho[0].estado.carrito.length && /nada más me quedan 40\. ¿Te pongo las 40\?/.test(mucho[0].texto), mucho[0].texto);
ok('y con «sí» pone las 40', mucho[1].estado.carrito[0]?.[1] === 40, JSON.stringify(mucho[1].estado.carrito));
...
const dedo = charla(['holaaa', 'ke onda tienes seras', 'la mate', 'kiero 2']);
ok('«holaaa» es un saludo', /¡Hola!/.test(dedo[0].texto), dedo[0].texto);
ok('«seras» encuentra las ceras (suena igual) y lo dice como sugerencia', /Creo que buscas/.test(dedo[1].texto) && /Cera Mate/.test(dedo[1].texto), dedo[1].texto);
ok('«kiero 2» es «quiero 2»', dedo[3].estado.carrito[0]?.[1] === 2, dedo[3].texto);
const foco = charla(['cuanto cuesta la maquina wahl', 'y si me das 3?']);
ok('«y si me das 3?» después de un precio: pone 3 de ese', foco[1].estado.carrito[0]?.join() === 'm1,3', foco[1].texto);
const nel = charla(['cuanto cuesta la maquina wahl', 'nel mejor no', 'gracias']);
ok('«nel mejor no» se respeta, no pasa a una persona', /Va\./.test(nel[1].texto) && !nel[1].estado.conPersona && !nel[1].estado.foco, nel[1].texto);
const risa = charla(['asdfghjkl', 'jajaja', '👍', '???']);
ok('risas, emojis y «???» no cuentan como no entender', risa.every((r) => !r.estado.conPersona) && risa.slice(1).every((r) => /En qué te ayudo/.test(r.texto)), risa.map((r) => r.texto).join(' | '));
const pulgar = charla(['cuanto cuesta la maquina wahl', '👍']);
ok('👍 a «¿te la aparto?» es un sí', pulgar[1].estado.carrito[0]?.join() === 'm1,1', pulgar[1].texto);
const cero = charla(['quiero 0 navajas', 'quiero -5 navajas']);
ok('«0» o «-5» no ponen nada: pregunta cuántas', cero.every((r) => !r.estado.carrito.length && /Cuántas/.test(r.texto)), cero.map((r) => r.texto).join(' | '));
const precio = charla(['precio']);
ok('«precio» a secas pregunta de qué', /De qué producto/.test(precio[0].texto) && !precio[0].estado.dudas, precio[0].texto);
const cuanto = charla(['cuanto cuesta la maquina wahl', 'cuanto es']);
ok('«cuánto es» sin pedido y con un producto en la plática: dice su precio', /\$2,450/.test(cuanto[1].texto), cuanto[1].texto);
const perro = charla(['tienen croquetas para perro']);
ok('lo que no vendemos no se disfraza de parecido', /No encontré/.test(perro[0].texto) && !/Creo que buscas/.test(perro[0].texto), perro[0].texto);
const sinPrecio = charla(['ke onda tienes seras']);
ok('la sugerencia por sonido sólo trae precios del catálogo', precios(sinPrecio[0].texto).every((x) => valido.has(x)), sinPrecio[0].texto);
