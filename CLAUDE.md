...
12. **Una guía es una foto, no el estado de las cosas.** Nombres de modelo, endpoints, banderas,
   ...
   **Y lo que yo "recuerdo" es la foto más vieja de todas.** Esta tabla es idea del Claude del
   ...
   | asignar `cv.width` sólo cambia el tamaño del lienzo | **lo BORRA**, aunque el número sea idéntico al que ya tenía | un letrero que crecía de dos renglones a tres movía la barra, el ResizeObserver disparaba `medir()` y la sala parpadeaba en negro **justo cuando la física iba lenta**, que es cuando el cuadro vacío se queda puesto lo bastante para verse |
   | una prueba que comprueba un gesto puede medir el resultado del gesto | el resultado depende de **dos** cosas —el gesto y el reglamento—, así que una prueba así no sabe cuál falló | «soltar aquí juega la carta» salió rojo 2 de 12 porque a veces la carta **no era legal**: el arrastre llegaba perfecto y la prueba culpaba al arrastre |
   | una corrida que no imprimió fallos es una corrida verde | también se ve así una que **no corrió**: maté sin querer el servidor de pruebas y ocho caídas seguidas se leyeron como ocho verdes | hay que exigir que aparezca el marcador, no que falte el ✗ |
   | `h >> 3` desplaza el número que tengo | sobre un entero **sin signo** de 32 bits lo lee **con** signo: da negativo la mitad de las veces | índice negativo → `undefined` → el dibujo de las personitas reventaba **cada cuadro**, y las 61 pruebas de pantalla seguían diciendo «ni un error de consola» porque **nunca ponían una personita** |
   | una prueba que arma el caso y pasa demuestra que la función sirve | demuestra que el motor **puede**, no que alguien pueda llegar ahí: la pistola disparaba en tres pruebas verdes mientras Carlos no lograba armar una | las ventanas de la receta son estrechas y no se adivinan dibujando — ánima 2-4, recámara ≥8, pólvora 3-6 |
   | «se sienta sola cuando le hablan» ya es una forma de darse de alta | sólo si hay una vía de hablarle que no pase por la lista — y la mesa arma esa lista **con el censo** | Negro y Paulina llevaban días existiendo con las llaves puestas y eran **inalcanzables desde el teléfono**: para aparecer había que hablarles y para hablarles había que aparecer. Carlos lo reportó como «no está ni Gemini ni groq» |
   ...
   | el navegador de este contenedor no sale a internet | no sale **solo**: con `ctx.route(/^https:/)` y cada petición reenviada desde Node por `request.newContext({ proxy })`, llega a la base real **sin apagar TLS** | la tienda se probaba con datos inventados y se decía «no se puede ver contra Supabase»; lanzar Chromium con el proxy da `ERR_CERT_AUTHORITY_INVALID` y la salida fácil era apagar la verificación |
   | `Intl.NumberFormat` con `notation:'compact'` escribe igual en todos lados | Node daba «$8.4 M» y el Chromium de las pruebas **«8.4 M$»**: cada motor trae sus propios datos de idioma | la cifra del inventario se veía rara en la captura y bien en `node -e`; se arma a mano cuando el ancho importa |
   ...
   | `flex: 1` reparte el ancho en partes iguales | es `flex: 1 1 0%` con `min-width: auto`: **nunca baja del ancho de su texto**, igual que `1fr` | «Transferencia» con su ícono se salía de la hoja de cobro a 390 px y ninguna prueba lo vio: el desborde era DENTRO de la hoja, no de la página |
   | el primer producto de la demo siempre tiene piezas | las pruebas venden de verdad contra la base de demo y **la van agotando** | la prueba sin red tocaba el primero, que ya estaba agotado, y tronó por un botón deshabilitado: las pruebas eligen `:not(.sin)` |
   | un `display: grid` sin columnas ya se acomoda al ancho | la columna implícita es `auto`: **crece al contenido más ancho**, igual que `1fr` | la parada del repartidor se salía 12 px con letra grande y nadie lo vio: las pruebas recorrían sólo el tema claro con letra normal. Se arregla con `grid-template-columns: minmax(0, 1fr)`, y `VARIANTE=oscuro,grande` recorre las rutas como las ve un señor grande de noche |
   ...
   | una fila con `overflow-x: auto` recorta todo lo que trae adentro | NO recorta a un hijo `position: absolute` cuyo bloque contenedor está afuera: el `.oculto` de los lectores de pantalla se quedaba en su lugar natural, 280 px a la derecha, y ensanchaba **la página entera** | la categoría medía 384 px en un teléfono de 320 y las pruebas de rutas no lo veían: miden a 390. Salió del «anciano» de `pruebas-personas.mjs`. `.oculto` va con `top:0; left:0` y las filas que se deslizan con `position: relative` |
   | si una pantalla pinta sin errores, sus botones funcionan | la prueba de rutas **sólo pinta**: el error de un botón sale al TOCARLO. `alMontar` recibía `{ aviso, ir }` y la hoja pedía `recargar` | «Editar» y «Nueva categoría» de la tienda **tronaban** y 109 rutas en verde decían que todo bien. Lo cazó el gato de `pruebas-personas.mjs`; ahora `pruebas-lint.mjs` (ESLint con `no-undef` y los nombres globales sacados del Chromium) lo ve sin tocar nada, en los 58 archivos |
   | dos «entrar» seguidos en la misma pestaña dejan la sesión del último que se pidió | dejan la del último que **terminó**: el armazón entraba solo como cajero al abrir el mostrador y el «ver como admin» de encima se cruzaba con él | la prueba de concurrencia pasaba **2 de 6** («Tu cuenta no puede hacer esto» a media prueba). `verComo` ahora va en fila, y `entrar` reintenta cuando otro teléfono le gana el token de la misma cuenta de muestra |
   | una regla más específica le gana a una genérica | gana **sólo en las propiedades que escribe**: `.grafica-horas .barra` ponía `height`, y la `.barra` de Reportes —mismo nombre, otro componente— traía `flex: 1 0 18px`, que nadie tapaba | en «Ventas de hoy» todas las horas medían lo mismo, al tope: el dato decía 6 % y la barra 146 px. Las pruebas veían que la gráfica existía, no cuánto medía cada barra |
   | «cambiar sólo los títulos» encuentra los títulos de un PowerPoint | los busca por el MARCADOR de título, y lo que sale de Canva, Google o un generador es todo cuadro de texto suelto | la operación dijo «listo» y tocó **cero** títulos en las dos presentaciones de Fadori. Sin marcador, cuenta como título el texto más grande de la lámina (`presentaciones/nucleo.js`) |
   | `el.replaceChildren(a, cond ? b : null)` se salta el que no va | el DOM **escribe** el `null` como texto: sólo nuestro `h()` se come los nulos | la barra de un elemento decía «● Color **null** ⧉ Duplicar»; lo vio la captura, no una prueba. Se filtra con `.filter(Boolean)` |
   | una variable puesta en el panel de Cloudflare se queda puesta | si es tipo **Text**, el siguiente `wrangler deploy` (cada fusión a main) **la borra**; sólo las **Secret** sobreviven. `keep_vars: true` lo evita | Paulina y Negro «apagados» otra vez tras fusionar el #127, con Carlos jurando que ya había puesto las llaves — y sí las había puesto |
   | `pkill -f guion.py` mata el guion viejo | mata **también el comando que lo está corriendo**: su propia línea contiene `guion.py`. Se lleva lo que venía después en el mismo comando | dos veces el 25 de septiembre: la corrección de `piezas.py` nunca se guardó y el guion siguió 5 minutos con la versión lenta. Se mata por número (`kill <pid>`) o con `pgrep -f "^python3 guion"` |
   | la letra de la casa (`mazi.woff2`) escribe cualquier frase | **no trae acentos**: pinta la vocal sin tilde, sin caer a otra letra | la portada de Presentaciones decía «Cambia cien laminas de un jalon». Mazi va sólo en la marca; las frases, en la letra de sistema |
   | `servidor.accept()` en un Durable Object sólo abre el socket | **cobra tiempo todo el rato que el socket está abierto**, aunque nadie hable; el plan gratis topa 13,000 GB-s y 100,000 renglones escritos al día y se reinicia a las 00:00 UTC (6 pm en México). Con `ctx.acceptWebSocket` el objeto duerme entre mensajes | la mesa y el banco daban «Load failed» en el iPhone y desde aquí todo contestaba bien: medí horas después, cuando para Cloudflare ya era otro día. Al toparse, la respuesta sale sin CORS y Safari sólo dice «Load failed» |
...
| `juegos/crisol/motor.js` | **La física de CRISOL**, aparte de la pantalla. Onda de choque con ecuación de onda, **cuerpos rígidos** que caen y chocan como piezas, **cámaras selladas con ley de los gases** —comprimir hace diamante, funde hidrógeno y revienta el bote—, campo gravitatorio por zonas y puntos, esfuerzos en compresión/tracción/corte, **voltaje** (apilar pilas calienta, mueve y lanza más), fricción, convección del aire, luz, compuertas lógicas y cuerdas con nudo | ✅ probada, 203/203 |
| `juegos/crisol/pruebas-pantalla.mjs` | **Los ojos de CRISOL.** Lo que Carlos reporta —«el menú se encima al suelo», «el botón de borrar no hace confirmación»— NO es física: es maqueta, y ninguna prueba de motor puede verlo. Comprueba el EFECTO de cada botón, no que exista. Corre en un navegador de verdad a 390×844 | ✅ 52/52 |
...
| `tienda/marca/cortar.py` + `piezas.py` + `tienda/nucleo/apertura.js` | **La apertura con logo.** Vuelan las **piezas sueltas que mandó Carlos** (G, D, B, navaja, peine y los dos mangos de las tijeras), enteras; `piezas.py` busca dónde cae cada una sobre el logo (giro, tamaño y lugar). Al aterrizar se funden con las capas cortadas del logo completo (`cortar.py`, cada pixel en UNA sola), así que el final es exactamente el logo. Una vez por sesión, se salta al toque, quieta con «reducir movimiento». Hoy: El Garaje del Barbero | ✅ 14/14 |
| `presentaciones/` | **Cambiar cien láminas de un jalón.** Abre un `.pptx` en el teléfono y cambia fondo, letra, color, tamaño, textos e imágenes en lote, con contraste automático y deshacer. Las imágenes se suben, se buscan en fotos libres (Openverse) o se hacen y rehacen con Gemini; Paulina y Negro proponen cambios de redacción que uno elige. Botones de **Acomodar** (que el texto quepa, fotos sin estirar, márgenes, alinear, títulos en su lugar, tamaños parejos, aire). **Recuadro detrás del texto** (cristal automático, claro, oscuro, sólido, píldora o su color; abraza al texto sin moverlo y se quita exacto). La IA tiene modo **«Opinión y consejos»**: recibe los textos y un informe de diseño MEDIDO del archivo, dice qué funciona, qué mejorar y qué apartados sumar, y «Aplícalo» lo vuelve cambios, incluidas láminas nuevas. Y el **banco de imágenes**: Carlos sube sus 300, Paulina escribe qué es cada una, temas y palabras clave, él marca «lista» o «requiere cambios» (y cuáles), se buscan sin acentos, se editan en lote, se exportan a Excel y se usan en las láminas. El banco vive en La Sala (`/banco`), no en el teléfono: Safari borra lo guardado tras 7 días sin abrir. La IA pasa por La Sala (`/ia-texto` con fotos, `/ia-imagen`), donde viven las llaves. **Insertar**: 20 formas, 1,854 iconos de Lucide que se buscan en español, 10 diseños armados y transiciones estándar, a una lámina, a las elegidas o a todas; en la vista grande se toca un elemento para moverlo, estirarlo, recolorearlo, duplicarlo o mandarlo al frente, y **▶ Presentar** las pasa a pantalla completa con sus transiciones. **Mis elementos**: lo que armes en una lámina, un dibujo con el dedo o un icono que haga la IA (sin fondo) se guarda en La Sala (`/elementos`) y se pone en cualquier presentación. Y la **campana** guarda cada cambio con su hora y «volver a antes de esto». **Tablas y gráficas nativas, como en Canva** (tamaño tocando la cuadrícula, 5 estilos, 6 tipos de gráfica con su hoja de Excel adentro para «Editar datos» en PowerPoint, y pegar celdas de Excel/Sheets en la tablita) y **enlaces** (texto o elemento que al tocarlo abre un link, también al presentar). El banco aguanta **300+ desde el iPhone**: reintenta, se reanuda sin repetir y mantiene la pantalla encendida | ✅ núcleo 84/84 · pantalla 133/133 · servidor 396/396 |
