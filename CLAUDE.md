...
- **`main` es lo que se publica**, **a propósito**, no por accidente. Pero el que publica ya no es
  GitHub Pages: **es Cloudflare**. `wrangler.jsonc` y `build.mjs` están en el repo justo para eso.
...
- **La dirección de verdad es `mazi-central.palomazi9111.workers.dev`**, no `github.io`.
  `ejercitopalomazi9111-arch.github.io/mazi-central` sigue en pie sólo mientras el repo sea
  público y **se apaga solo** en cuanto Carlos lo pase a privado —servir un repo privado es
  función de paga de GitHub, y Cloudflare sí lo hace gratis: por eso la mudanza—. Así que **no se
  escribe `github.io` en ningún metadato, manual ni enlace**: ya pasó, y hubo que corregir siete
...
- Los pasos que sólo puede dar Carlos —crear el proyecto en Cloudflare, conectar el servidor,
  ...
  Si el despliegue todavía no está en pie o el repo sigue público, es que falta un paso de ahí, no
  que esto esté mal configurado.
